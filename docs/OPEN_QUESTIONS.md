...
| What derives a drawing`s identity? | **Content — `sha256`.** Never filename, upload event or folder structure: the same evidence must not create multiple drawing identities, because duplicates inflate evidence aggregation. Filename, revision and upload metadata are kept separate from identity, and `drawingIdDerivedFrom` records which rule produced a given id. **Not yet implemented, and blocked on a second decision:** `facilityOf` reads the site out of the id string and `planOf` groups the `.dwg`/`.pdf` twins, so a bare hash makes every drawing its own facility and every export its own plan — inverting D6 and D15, which is the same inflation D17 exists to prevent, moved from identity into grouping. Facility and plan must become recorded fields rather than substrings of the id; that is a schema change plus regeneration of 300 catalogue entries and 306 corpus rows, and the migration of existing path-derived ids is itself a decision. | D17 · raised in `pilot/PILOT-001/input/BLOCKED.md`, tabulated in `docs/PILOT_VALIDATION_PROCESS.md` §2 |
