...
```
Project
 └─ Level                    a floor of the building
     ├─ planImage            the imported PDF / PNG / JPG
     ├─ scale                pixels → millimetres calibration
     └─ Space                a room
         └─ Placement        one machine on the drawing
             └─ Port         power · RO water · drain
```

`Placement` points at an **Equipment Object** in the catalogue — it does not copy it. See
...
| `scale` | ScaleCalibration \| null | **Null until calibrated.** |
| `spaces` | Space[] | |
...
| `transform` | Transform | Where the image sits in model space |

### ScaleCalibration

| Field | Type | Notes |
| --- | --- | --- |
| `pointA` / `pointB` | Vec2 | Two points the engineer picked on the plan, in image pixels |
| `knownDistance` | Millimetres | The real distance the engineer typed |
| `millimetresPerPixel` | number | Derived, stored for traceability |
| `method` | `two-point` \| `stated-ratio` | |
| `calibratedAt` | timestamp | |

**An uncalibrated Level cannot be validated.** Any rule evaluated against a plan with
`scale === null` returns YELLOW with the reason "plan not calibrated" — never GREEN.
Measuring screen distance and calling it a clearance is the single most damaging thing
this application could do, so it is blocked structurally rather than by a warning.
...
| Level, PlanImage, ScaleCalibration | 4 |
