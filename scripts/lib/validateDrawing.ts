...
/** Centre-to-centre spacing the layout is built on, taken from the sheet's own printed dimension. */
const STATION_PITCH_MM = 2_000;
...
export async function validateDrawing(input: ValidationInput): Promise<ValidationOutcome> {
  ...
  // ── place ────────────────────────────────────────────────────────────────────
  const pitchDimension = agreement.consistent.find(
    (entry) => entry.statedMm === STATION_PITCH_MM,
  );
  if (!pitchDimension) {
    return stop({
      ...base,
      ...
      discrepancies: [
        ...
        discrepancy(
          'VD-7',
          'insufficient_evidence',
          'station pitch is not printed',
          `The sheet prints no ${STATION_PITCH_MM.toLocaleString('en')} mm dimension that agrees with its own ` +
            'scale, so the spacing a layout would be built on is not stated. Nothing is assumed ' +
            'in its place.',
        ),
      ],
    });
  ...
  for (let row = 0; row < 2; row += 1) {
    for (let index = 0; index < STATIONS_PER_ROW; index += 1) {
      const x = index * pitchDimension.statedMm;
      placements.push({
        id: `bed-${row}-${index}`,
  ...
  const observations: Observation[] = [
    {
      id: `${input.drawingId}#station_pitch`,
      source: {
        drawing: drawingRef,
        ...
        confidence: 'high',
        note:
          `Printed as "${pitchDimension.label}". Its geometry measures ` +
