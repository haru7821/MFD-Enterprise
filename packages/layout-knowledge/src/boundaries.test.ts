...
describe('the shipped knowledge base', () => {
  ...
  it('records the analyst as a model, not as a person', () => {
    /*
     ...
     *
     * The single assertion that keeps that true for every figure in the base. If these were ever
    ...
    for (const observation of observations) {
      ...
      expect(observation.source.observer.name, observation.id).toBe('MFD Hospital Dataset Analyzer');
      expect(observation.source.observer.version, observation.id).toBe('1.0');
    }
  });
});
