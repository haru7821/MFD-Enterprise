...
describe('the shipped knowledge base', () => {
  ...
  it('carries the full provenance the validation programme requires, on every observation', () => {
    /*
     * > Owner decision, validation programme: an observation must include *"drawing SHA-256, drawing
     * > identifier, page, observation type, measured value, measurement method, observer,
     * > confidence."*
     *
     * The schema requires each of these, so this is belt and braces — but it is the *list* that is
     * the decision, and a list held only by a schema is one that quietly loses an entry when the
     * schema is refactored. Asserted as eight named things so removing one is a visible edit here
     * rather than an invisible one there.
     */
    ...
    for (const observation of observations) {
      const { drawing, method, observer, confidence } = observation.source;
      expect(drawing.sha256, observation.id).toMatch(/^[a-f0-9]{64}$/);
      expect(drawing.drawingId.length, observation.id).toBeGreaterThan(0);
      // Zero is a real answer. Every drawing in this dataset is one sheet; a drawing set would not
      // be, and then which page a reading came from is the only way to find it again.
      expect(Number.isInteger(drawing.page), observation.id).toBe(true);
      expect(observation.value.kind.length, observation.id).toBeGreaterThan(0);
      expect(method.length, observation.id).toBeGreaterThan(0);
      expect(observer.name.length, observation.id).toBeGreaterThan(0);
      expect(confidence.length, observation.id).toBeGreaterThan(0);
    }
  });
